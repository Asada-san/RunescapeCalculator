...
class Ability:
    ...
    def StandardChannelDamAvgCalc(self, player, Do, Type=None, IDX=None, PunctureStack=0):
        ...
        :param PunctureStack: Puncture stack of the Greater Dazing Shot ability.
        ...
        if self.Name != 'Bash':  # Bash ability has its own base damage
            Max = Max * player.BaseDamageEffective * (1 + player.Rr * 0.025) * player.BerserkersFury
            Min = Min * player.BaseDamageEffective * (1 + player.Rr * 0.025) * player.BerserkersFury
        else:
    ...
    def BleedDamAvgCalc(self, player, Do):
        ...
        ################## Calculate averages ########################
        if self.Name in {'Massacre', 'Deadshot'}:  # Simply bleed max / amount of hits
        ...
        elif self.Name in {'Blood Tendrils', 'Corruption Shot', 'Corruption Blast', 'Incendiary Shot'}:  # Simply max + min / 2
        ...
        elif self.Name in {'Death\'s Swiftness', 'Sunshine'}:  # Simply avg of max + min
