            ...
            if (loop.ConditionList[i][0] == loop.ConditionList[-1][0]).all():
                ...
                if loop.ConditionList[i][2] == loop.ConditionList[-1][2]:
                    ...
                    if loop.ConditionList[i][3] == loop.ConditionList[-1][3]:
                        ...
                        if Do.HTMLwrite:
                            Do.Text += f'<li style="color: {Do.cycle_color};">CYCLE FOUND, EXTENDING RUN BY 1x CYCLETIME: {round(loop.CycleTime * .6, 1)}s</li>\n'
...
def CycleRotation(bar, player, dummy, Do, loop):
    ...
    elif loop.n == loop.nMax: # If the max runtime has been reached
        for attr, value in player.AbilInfo.items():
            if value['activations'] == 0:
                loop.Redundant.extend([attr])
